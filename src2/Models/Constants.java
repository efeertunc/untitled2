package Models;

public interface Constants {

    interface DatabaseConstants {
        String LOGIN_ACCEPTED = "Login accepted";
        String LOGIN_REJECTED = "Login rejected";
        String REGISTER_ACCEPTED = "Register successfully";
        String REGISTER_REJECTED = "Register failed";
        String DATABASE_WRITE_ERROR = "Database write error has occured";
        String DATABASE_READ_ERROR = "Database read error has occured";
        String JSON = "src2/Utilities/DBManager/comp302-4de74-firebase-adminsdk-yfz83-9552d14b68.json";
    }

}
